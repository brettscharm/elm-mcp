    ...
    if name == "generate-requirements":
        ...
        return [PromptMessage(
            ...
            content=TextContent(type="text", text=(
                ...
                f"The user told you to ask MANY questions before "
                f"generating anything. The system description above is "
                f"a starting point, NOT a complete spec. Run a deep "
                f"interview, ONE QUESTION AT A TIME, before writing any "
                f"shall-statements. Minimum coverage:\n\n"
                f"  1. **Primary user + secondary users.** Who, doing what?\n"
                f"  2. **Context.** Greenfield? Replacing? Augmenting?\n"
                f"  3. **Tech stack constraints.** Lang? Framework? "
                f"Deploy target? Version locks?\n"
                f"  4. **Performance targets** — push for measurable "
                f"numbers, not 'fast'. p95 latency in ms, RPS, data "
                f"volume.\n"
                f"  5. **Scale** — concurrent users? peak vs steady?\n"
                f"  6. **Integrations** — which external systems? "
                f"protocols? auth?\n"
                f"  7. **Data** — sources? schemas? retention? PII?\n"
                f"  8. **Security / threat model** — what are you "
                f"defending against?\n"
                f"  9. **Observability** — metrics, logs, SLOs?\n"
                f"  10. **Failure modes** — what MUST keep working?\n"
                f"  11. **AC style** — Given/When/Then? prose? numeric?\n"
                f"  12. **Out-of-scope** — what should NOT be in this "
                f"batch? (Often the most-missed.)\n\n"
                f"**Follow up on every vague answer.** Don't accept "
                f"'fast' / 'secure' / 'scalable' without measurable "
                f"criteria. If user says 'I don't know', offer 3-4 "
                f"concrete options and have them pick.\n\n"
                f"After the interview:\n\n"
                f"1. `get_artifact_types` to learn the project's shape "
                f"vocabulary.\n"
                f"2. Generate {count} requirements following IEEE 29148 "
                f"/ INCOSE — 'shall' for mandatory, atomic, testable, "
                f"measurable acceptance criteria drawn from YOUR "
                f"interview answers (NOT hallucinated thresholds), "
                f"grouped by functional area.\n"
                f"3. Preview in a clean table — Type | Title | "
                f"Acceptance Criteria.\n"
                f"4. Wait for explicit approval. Edits → re-preview.\n"
                f"5. On approval, `create_requirements` with "
                f"`module_name` set so reqs auto-bind.\n\n"
                f"Better to ask 15 questions and generate accurate "
                f"requirements than ask 3 and hallucinate plausible-"
                f"but-wrong ones."
            )),
        )]
...
async def _dispatch_tool(name: str, arguments: Any) -> list[TextContent]:
    ...
    try:
        ...
        if name == "build_project_next":
            ...
            phase_scripts = {
                1: ("PHASE 1 — PROJECT INTAKE INTERVIEW (DEEP)",
                    "Ask the user 12–18 specific questions, ONE AT A "
                    "TIME. Wait for each answer before the next. "
                    "**Follow up on every vague answer.** The user "
                    "told you explicitly to ask many questions before "
                    "generating — honor that.\n\n"
                    "Cover ALL of these (skip ONLY if the user's "
                    "initial idea statement already definitively "
                    "answers one):\n"
                    "  1. **One-paragraph description** — what does "
                    "the user DO with this thing? Who's primary user?\n"
                    "  2. **Context** — replace something? augment? "
                    "greenfield? What triggered this work?\n"
                    "  3. **Tech stack** — lang? framework? runtime? "
                    "deploy target? Follow up: versions? cloud/on-prem?\n"
                    "  4. **Scale** — concurrent users? RPS? data "
                    "volume? Peak vs steady-state?\n"
                    "  5. **Performance** — latency / throughput. "
                    "Push for p50/p95/p99 in ms, not just 'fast'.\n"
                    "  6. **Integrations** — which external systems? "
                    "protocols (REST/gRPC/MQ)? auth model?\n"
                    "  7. **Data** — sources? schemas? retention? "
                    "PII? encryption at-rest/in-transit?\n"
                    "  8. **Standards / compliance** — regulated? "
                    "DO-178C / ISO 26262 / NIST / HIPAA / PCI / GDPR / "
                    "none? Follow up on audit cadence.\n"
                    "  9. **Security** — threat model? secrets mgmt? "
                    "key rotation cadence?\n"
                    "  10. **Observability** — metrics, logs, traces? "
                    "SLO targets? alerting thresholds?\n"
                    "  11. **Failure modes** — what MUST keep working "
                    "if X breaks? graceful degradation strategy?\n"
                    "  12. **AC style** — Given/When/Then? plain "
                    "prose? numeric thresholds?\n"
                    "  13. **Scope / count** — handful (5–10), "
                    "moderate (15–25), comprehensive (30+)?\n"
                    "  14. **Out-of-scope** — what should NOT be in "
                    "this? (Most-missed; push hard.)\n"
                    "  15. **Known unknowns** — what does the user "
                    "want help deciding?\n\n"
                    "**Vague-answer rule:** if user says 'fast' / "
                    "'secure' / 'scalable' / 'reliable', do NOT accept "
                    "and move on. Push for measurable:\n"
                    "  - 'fast'     → 'p95 latency target in ms?'\n"
                    "  - 'secure'   → 'threat model? PII? PCI? GDPR?'\n"
                    "  - 'scalable' → 'concurrent users? RPS?'\n"
                    "  - 'reliable' → 'uptime target? RPO/RTO?'\n\n"
                    "**'I don't know' rule:** don't skip. Offer 3-4 "
                    "concrete options and have them pick. The picking "
                    "IS the decision.\n\n"
                    "After all questions, summarize the scope back in "
                    "one paragraph reflecting EVERY answer. Wait for "
                    "explicit approval. Only then call "
                    "`build_project_next(current_phase=1, "
