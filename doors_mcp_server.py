...
async def get_prompt(name: str, arguments: dict | None = None) -> list[PromptMessage]:
    ...
    # Use /build-new-project (greenfield) or /build-from-existing (brownfield).
        ...
        workflow = (
            ...
            "6. **Interview discipline** (v0.7.0 per-artifact "
            "gates). Jira ticket = SEED material, not full set. "
            "Re-elicit.\n\n"
            "7. **Preview** — counts + every item in full + "
            "Jira source per item.\n\n"
            "8. **Wait for explicit approval.**\n\n"
            "9. **Push to DNG via `create_requirements`** with "
...
_REQ_GATE = (
    ...
    "(1) INTERVIEWED — run the **Step 3b** question set in BOB.md "
    "(12–15 areas: primary user + #1 job, context new/replace/augment, "
    "tech stack + deploy target, performance in ms, scale users/RPS, "
    "integrations + protocols, data + PII handling, regulations, "
    "security threat model, observability, failure modes, AC style, "
    "scope count, and EXPLICIT out-of-scope). ONE question at a time. "
    "Verbatim shape: `[topic] — plain question / Examples: a, b, c / "
    "(Why I'm asking: ...) / If unsure: [safe default]`. Wait for each "
    "answer. Follow up on every vague answer ('fast' → p95 in ms; "
    "'secure' → threat model; 'scalable' → concurrent users + RPS). "
    "(2) PREVIEWED — Markdown table with Type | Title | Acceptance "
    "Criteria | Link target — BEFORE this tool fires. Re-preview after "
    "any edit. "
    "(3) CONFIRMED — explicit 'yes / ship it / go ahead / push them'. "
    "The user saying 'I need some requirements' is a REQUEST, not "
    "approval. Hard rule: under 8 specific questions across the Step 3b "
    "areas = NOT enough — push for more. Hallucinated NFRs ship and "
    "break things. — "
)
...
_WORKITEM_GATE = (
    ...
    "DO NOT CALL THIS TOOL until you have, in order: "
    "(1) INTERVIEWED — run the **Step 3d** question set in BOB.md "
    "(8–12 areas: decomposition strategy, cross-cutting work — CI/CD, "
    "infra, observability, secrets — effort/estimate model, foundation "
    "set, spike vs build, Definition of Done, owner/assignment, risk "
    "flags, iteration target, team area, extra links, and EXPLICIT "
    "out-of-scope tasks). For defects also ask: repro steps, severity, "
    "environment, found-in version, affected-users impact. ONE question "
    "at a time. Verbatim shape: `[topic] — plain question / Examples / "
    "(Why) / If unsure`. Follow up on vague answers ('small' → 2 hours "
    "or 2 days?; 'just do it' → what does Resolved look like?). "
    "(2) PREVIEWED — table with Title | Covers Req | Est | Owner | Deps "
    "| Risk | DoD — BEFORE this tool fires. Re-preview after any edit. "
    "(3) CONFIRMED — explicit approval. "
    "ALWAYS pass `requirement_url` when the work item implements a DNG "
    "requirement — without it, traceability breaks. Hard rule: under 6 "
    "specific questions across the Step 3d areas = NOT enough. — "
)

_TESTCASE_GATE = (
    "🛑 TEST CASE WRITE GATE — Bob's #1 rule. "
    "DO NOT CALL THIS TOOL until you have, in order: "
    "(1) INTERVIEWED — run the **Step 3e** question set in BOB.md "
    "(8–12 areas: test levels in scope — unit/integration/system/"
    "acceptance/perf/security, automation strategy + framework, coverage "
    "per req — happy/negative/boundary, edge cases — empty/max-length/"
    "malformed/unicode/concurrent/downstream-down/expired-auth, test "
    "data source, environment + mocks + feature flags, pass/fail style — "
    "MEASURABLE, NFR thresholds + load profile, security tests per "
    "threat, Given/When/Then mapping, traceability granularity, and "
    "EXPLICIT out-of-scope tests). ONE question at a time. Verbatim "
    "shape: `[topic] — plain question / Examples / (Why) / If unsure`. "
    "Follow up on vague answers ('works' → what's the measurable "
    "criterion?; 'should fail' → with what error code/message?; "
    "'reasonable' → p95 < what ms?; 'normal data' → name 3 "
    "representative records). "
    "(2) PREVIEWED — table with Title | Covers Req(s) | Pos/Neg/"
    "Boundary/Perf | Env | Automated? | Pass criterion — BEFORE this "
    "tool fires. Re-preview after any edit. "
    "(3) CONFIRMED — explicit approval. "
    "ALWAYS pass `requirement_url` when the test validates a DNG "
    "requirement. Hard rule: under 6 specific questions across the "
    "Step 3e areas = NOT enough. Bad tests catch nothing. — "
)
